...
export async function POST(request: NextRequest) {
  try {
    const { text, difficulty = "medium", questionCount = 5, questionType = "mixed" } = await request.json()
...
- options: array of option strings (only for multiple-choice questions)
- correctAnswer: the correct answer (letter for multiple-choice, "True"/"False" for true-false, text for others)
