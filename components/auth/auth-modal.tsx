...
import { signUpWithEmail, signInWithEmail, signInWithGoogle, signInWithApple } from "@/lib/firebase/auth"
...
export function AuthModal({ isOpen, onClose, onSuccess, initialMode = "signin" }: AuthModalProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      setGoogleError("An unexpected error occurred with Google sign-in")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleAppleAuth = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          title: "Welcome!",
          description: "You've been signed in with Apple successfully!",
        })
        onSuccess?.()
        onClose()
      }
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              <Button
                variant="outline"
                className="w-full bg-white hover:bg-gray-50 text-gray-900 border-gray-300 shadow-sm font-medium"
              ...
              {googleError && <p className="text-sm text-red-600 px-1">{googleError}</p>}
            </div>
